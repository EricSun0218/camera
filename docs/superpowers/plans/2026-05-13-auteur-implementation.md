...
**Goal:** Build the Auteur iOS app and its Cloudflare Worker backend per `docs/superpowers/specs/2026-05-13-auteur-design.md`. Deliverable is "ready to open in Xcode and deploy to Cloudflare" — no Xcode is available in this environment, so iOS verification is deferred to the user.
...
- iOS tasks (Swift) — cannot be compiled in this environment (only Apple Command Line Tools, no Xcode/iOS SDK). Each Swift task ends in "Commit". The user verifies by opening `app/Auteur.xcodeproj` in Xcode after install.
...
**Files:**
- Create: `app/Auteur/Color/GradeParams.swift`
- Create: `app/Auteur/Models/SceneAnalysis.swift`
- Create: `app/Auteur/Models/CoachTip.swift`
- Create: `app/Auteur/Color/NeutralPreset.swift`
...
```swift
// app/Auteur/Models/SceneAnalysis.swift
import Foundation
...
```swift
// app/Auteur/Color/NeutralPreset.swift
import Foundation
...
```bash
git add app/Auteur/Color/GradeParams.swift app/Auteur/Color/NeutralPreset.swift \
        app/Auteur/Models/SceneAnalysis.swift app/Auteur/Models/CoachTip.swift
...
**Files:**
- Create: `app/Auteur/Color/HSLKernel.swift`
- Create: `app/Auteur/Color/HSLKernel.ci.metal`

- [ ] **Step 1: Metal kernel source**

```metal
// app/Auteur/Color/HSLKernel.ci.metal
...
```swift
// app/Auteur/Color/HSLKernel.swift
import CoreImage
import Foundation
...
```bash
git add app/Auteur/Color/HSLKernel.swift app/Auteur/Color/HSLKernel.ci.metal
...
**Files:**
- Create: `app/Auteur/Color/CIPipeline.swift`

- [ ] **Step 1: Pipeline**

```swift
// app/Auteur/Color/CIPipeline.swift
import CoreImage
...
```bash
git add app/Auteur/Color/CIPipeline.swift
...
**Files:**
- Create: `app/Auteur/Color/PhotoRenderer.swift`

- [ ] **Step 1: Implementation**

```swift
// app/Auteur/Color/PhotoRenderer.swift
import CoreImage
import ImageIO
...
```bash
git add app/Auteur/Color/PhotoRenderer.swift
...
**Files:**
- Create: `app/Auteur/LLM/ImageEncoder.swift`

- [ ] **Step 1: Implementation**

```swift
// app/Auteur/LLM/ImageEncoder.swift
import CoreImage
import ImageIO
import UniformTypeIdentifiers
import UIKit
...
```bash
git add app/Auteur/LLM/ImageEncoder.swift
...
**Files:**
- Create: `app/Auteur/LLM/BackendClient.swift`
...
```bash
git add app/Auteur/LLM/BackendClient.swift
...
**Files:**
- Create: `app/Auteur/Compose/OnDeviceCV.swift`
...
```bash
git add app/Auteur/Compose/OnDeviceCV.swift
...
**Files:**
- Create: `app/Auteur/Compose/CoachThrottler.swift`

- [ ] **Step 1: Implementation**

```swift
// app/Auteur/Compose/CoachThrottler.swift
import Foundation
import CoreImage
...
```bash
git add app/Auteur/Compose/CoachThrottler.swift
...
**Files:**
- Create: `app/Auteur/Compose/CompositionOverlay.swift`
...
```bash
git add app/Auteur/Compose/CompositionOverlay.swift
...
**Files:**
- Create: `app/Auteur/Camera/CameraSession.swift`

- [ ] **Step 1: Implementation**

```swift
// app/Auteur/Camera/CameraSession.swift
import AVFoundation
import CoreImage
import Combine
import UIKit
...
public final class CameraSession: NSObject {
    ...
    private let sessionQueue = DispatchQueue(label: "auteur.camera.session")
    ...
    public func configure() {
        sessionQueue.async { [weak self] in
            ...
            do {
                ...
                self.videoOutput.setSampleBufferDelegate(self, queue: DispatchQueue(label: "auteur.camera.video"))
...
```bash
git add app/Auteur/Camera/CameraSession.swift
...
**Files:**
- Create: `app/Auteur/Camera/CameraPreviewView.swift`

- [ ] **Step 1: UIViewRepresentable**

```swift
// app/Auteur/Camera/CameraPreviewView.swift
import SwiftUI
import AVFoundation
...
```bash
git add app/Auteur/Camera/CameraPreviewView.swift
...
**Files:**
- Create: `app/Auteur/App/AuteurApp.swift`
- Create: `app/Auteur/App/RootView.swift`
- Create: `app/Auteur/Views/PermissionGate.swift`
...
```bash
git add app/Auteur/App/AuteurApp.swift app/Auteur/App/RootView.swift app/Auteur/Views/PermissionGate.swift
...
**Files:**
- Create: `app/Auteur/Views/BeforeAfterReveal.swift`
...
```bash
git add app/Auteur/Views/BeforeAfterReveal.swift
...
- Create: `app/Auteur/Resources/Info.plist`
- Create: `app/Auteur/Resources/Assets.xcassets/Contents.json`
- Create: `app/Auteur/Resources/Assets.xcassets/AppIcon.appiconset/Contents.json`
- Create: `app/Auteur/Resources/Assets.xcassets/AccentColor.colorset/Contents.json`
...
targets:
  Auteur:
    ...
    sources:
      - path: Auteur
    info:
      path: Auteur/Resources/Info.plist
      properties:
        CFBundleDisplayName: Auteur
        UILaunchScreen: {}
        UIRequiresFullScreen: YES
        UISupportedInterfaceOrientations: [UIInterfaceOrientationPortrait]
        NSCameraUsageDescription: "Auteur 需要相机来拍摄并实时给出构图与调色建议。"
        NSPhotoLibraryAddUsageDescription: "Auteur 将自动调色后的照片保存到你的相册。"
        ITSAppUsesNonExemptEncryption: false
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: com.auteur.app
        ASSETCATALOG_COMPILER_APPICON_NAME: AppIcon
        ASSETCATALOG_COMPILER_GLOBAL_ACCENT_COLOR_NAME: AccentColor
        DEVELOPMENT_TEAM: ""    # set in Xcode
    resources:
      - path: Auteur/Resources/Assets.xcassets
...
```json
// app/Auteur/Resources/Assets.xcassets/Contents.json
{
...
```json
// app/Auteur/Resources/Assets.xcassets/AppIcon.appiconset/Contents.json
{
  "images" : [
...
```json
// app/Auteur/Resources/Assets.xcassets/AccentColor.colorset/Contents.json
{
  "colors" : [
    {
      "color" : {
...
Expected: `Auteur.xcodeproj` directory exists.

- [ ] **Step 5: Commit**

```bash
git add app/project.yml app/Auteur/Resources/
...
The `.ci.metal` file at `app/Auteur/Color/HSLKernel.ci.metal` is picked up by xcodegen's `sources:` (recursive). Xcode auto-compiles `.metal` into the default Metal library at `default.metallib`. The Swift loader in `HSLKernel.swift` already falls back to `default.metallib`. No further change needed beyond xcodegen including the path.

Verify the path is included:

```bash
cd app && xcodegen generate
grep -c 'HSLKernel.ci.metal' Auteur.xcodeproj/project.pbxproj
```
...
```yaml
    sources:
      - path: Auteur
      - path: Auteur/Color/HSLKernel.ci.metal
        type: file
```
...
**Files:**
- Create: `app/AuteurTests/GradeParamsTests.swift`
...
```bash
git add app/AuteurTests/GradeParamsTests.swift
...
**Files:**
- Create: `app/AuteurTests/CIPipelineTests.swift`

- [ ] **Step 1: Test**

```swift
// app/AuteurTests/CIPipelineTests.swift
import Testing
import CoreImage
@testable import Auteur
...
```bash
git add app/AuteurTests/CIPipelineTests.swift
...
```bash
cd /Users/blink/project/auteur/app && xcodegen generate
```

Expected: regenerates `Auteur.xcodeproj` without errors.

- [ ] **Step 2: List final tree**

```bash
cd /Users/blink/project/auteur && find . -type f -not -path './.git/*' -not -path '*/node_modules/*' -not -path '*/.wrangler/*' -not -path '*xcuserdata*' | sort
```
