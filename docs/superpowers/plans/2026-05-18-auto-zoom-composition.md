...
- **The Xcode project is generated by XcodeGen** from `app/project.yml`; `Cue.xcodeproj` is gitignored. Never edit or commit `project.pbxproj`. Both targets glob their source folders (`sources: - path: Cue` / `- path: CueTests`), so a new file is picked up by running `cd app && xcodegen generate`. Only commit `.swift` / `project.yml` files.
- All `xcodebuild` commands append `CODE_SIGNING_ALLOWED=NO` (the simulator test host otherwise hits a codesign error).
...
> **Done.** Implemented in commit `fd90dde`. The registration step originally
> here (editing `project.pbxproj`) was wrong — the project is XcodeGen-generated.
> The correct step is: create the file, then `cd app && xcodegen generate`.
